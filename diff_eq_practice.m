...
figure("Name","First pendulum example","Position",[100 100 1000 500])
fplot(matlabFunction(sol_pend), [0, 20])
xlabel('Time t')
ylabel('theta(t)')
...
syms y_ex(t)
Dy_ex = diff(y_ex,t);
D2y_ex = diff(y_ex,t,2);
D3y_ex = diff(y_ex,t,3);

eqn_ex = D3y_ex + 10*D2y_ex + 6*Dy_ex + 2*y_ex == 0;
conds_ex = [y_ex(0)==-1, Dy_ex(0)==-1, D2y_ex(0)==1];
sol_ex = dsolve(eqn_ex,conds_ex);

% Plot ODE from practice exam
figure("Name","Practice Exam")
fplot(matlabFunction(sol_ex))
xlabel('Time t')
ylabel('y(t)')
title('Practice Exam ODE')
grid on
hold off
